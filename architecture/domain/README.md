...
[`system-evidence.yml`](../references/system-evidence.yml) is evidence for the
original governance and Control Plane language. This Architecture register
adds the distinct catalog/release and bootstrap/bundle models evidenced by
the Primitive and Distribution contracts, plus the context-steward review
role. The bounded-context register maps each context to its stewarding
repository and pinned evidence path. That mapping routes semantic review; it
does not name a person or grant GitHub approval.
...
conformance rules, and organization-wide decision text, including records
whose domain scope is context-local. It is not a fifth bounded context: this
repository does not establish a separate business model or
...
| Context | Owns | Stewarding repository | Evidence and boundary |
| --- | --- | --- | --- |
| `agentic-delivery-governance` | Repository delivery governance and reusable rules for proposing, reviewing, validating, and recording work | `agentic-delivery-lab/agentic-delivery` | Defined in the Control Plane register at the audited baseline. The repository-level control plane is a GitHub surface; it is not the executable controller. |
| `agentic-delivery-control-plane` | The delivery controller, event intake, enrollment, lifecycle contracts, orchestration, and controlled GitHub write-back | `agentic-delivery-lab/agentic-delivery` | Defined in the Control Plane register; runtime implementation remains in `agentic-delivery`. It does not own Architecture or Primitive implementation. |
| `agentic-primitives` | Versioned agents, skills, instructions, hooks, validators, capabilities, and MCP contracts, with a catalog and release/projection rules | `agentic-delivery-lab/agentic-delivery-primitives` | Distinct catalog and release model in ADP-0001 and `manifests/primitive-catalog.yml` at the pinned Primitive revision. |
| `developer-distribution` | Reproducible development-environment and thin consumer integration bundles, bootstrap, and consumer projections | `agentic-delivery-lab/agentic-delivery-distribution` | Distinct bundle/bootstrap model in ADD-0001 and `manifests/workflow-bundle.json` at the pinned Distribution revision. |
...
The repository mapping and its evidence paths are checked against
[`system-evidence.yml`](../references/system-evidence.yml). They route semantic
review requests, but do not name a person or prove CODEOWNERS coverage. The
audited one-member/CODEOWNERS gate currently limits independent steward
approval; see parent issue [#59](https://github.com/agentic-delivery-lab/agentic-delivery/issues/59).
...
pinned Primitive catalog and resolves every ADR to one local Architecture
record. The decision inventory pins imported source identity, commit, path, and
SHA-256 while keeping a single canonical prose file per ID.
...
- The [canonical decision inventory](../references/decision-inventory.yml) records all text IDs, bounded-context scope, imported source pins, and differing historical variants. Consumer copies remain at their audited revisions until each repository replaces them with release-pinned references in its own reviewed change.
- [ADP-0001](../../decisions/ADP-0001-primitive-release-and-projection.md) and its source-pinned catalog define the Primitive context; its origin is recorded in the inventory.
- [ADD-0001](../../decisions/ADD-0001-distribution-boundary.md) and its source-pinned bundle define the Distribution context; its origin is recorded in the inventory.
