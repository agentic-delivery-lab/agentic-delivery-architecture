...
| ADR-0018 Organization-wide Control Plane distribution and versioning | One organization App installation intended for `All repositories`, a separate reviewed participant registry, origin-scoped tokens, immutable controller pins, and no credential access from model tools | `api/github/webhook.mjs`; `scripts/lib/github-app.mjs`; `config/participants.yml`; central invocation/intake/delivery workflows; offline multi-repository acceptance and auth tests | `pnpm acceptance:check` and the four-root release-chain check pass as offline evidence; the fixture's second repository and `.github-private` IDs are synthetic | The operator reports `All repositories`, but App-installation inspection through the available OAuth identity returns 401; the current App contract still says `selected-repositories`; Projects inventory lacks `read:project` | Live App settings, event subscriptions, token refresh, organization issue-field GraphQL writes, production event delivery, and production write-back remain unobserved | After ADR-0018 is approved, align and test the Control Plane contract; an App-authorized owner must verify settings and a shadow smoke run before activation | deterministic security boundary; live runtime unverified | architecture gate in progress; all registered participants remain shadow | keep the single Control Plane in `agentic-delivery`; do not activate production mutation or private-surface publication until the separately reviewed gates pass |
| ADR-0019 Canonical delivery-state field | Lifecycle Stage and the orthogonal Delivery State are distinct; current Delivery Readiness is a legacy name; no silent live-field migration or duplicate field | ADR, domain register, issue metadata contract, compatibility release validator, migration tests | `decisions/0019-canonicalize-delivery-state-field.md`; `architecture/domain/ubiquitous-language.yml`; Delivery Control Plane `config/issue-metadata.yml`; `config/controller-release.json`; control-plane migration tests | Live field rename and option-ID inventory are unavailable in this environment | Current organization field is still named Delivery Readiness and remains authoritative until an operator migration | GraphQL inventory, plan entitlement, rename capability, shadow comparison, and rollback evidence are required before activation | deterministic contract plus semantic decision; live migration unverified | provisional and awaiting operator field migration | retain the readiness alias until every participant upgrades; do not create a second field |
...
the Delivery Control Plane's `config/issue-metadata.yml` and
`config/orchestration-policy.yml`. Native
...
The Control Plane's `config/agent-actors.json` defines the `@agentic-delivery-lab-invoker-7f3a` mention, the
