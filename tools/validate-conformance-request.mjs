...
export function validateConformanceRequest(request, expectedArchitectureCommit = null, expectedArchitectureDigest = null) {
  const errors = [];
  ...
  if (!SHA256.test(request?.architectureDigest ?? '')) errors.push('request architectureDigest must be an immutable SHA-256 digest');
  ...
  if (expectedArchitectureDigest !== null && request?.architectureDigest !== expectedArchitectureDigest) errors.push('request architectureDigest does not match the checked-out Architecture Authority');
  if (errors.length) throw new Error(`architecture conformance request validation failed:\n${errors.join('\n')}`);
  return { schemaVersion: 1, architectureCommit: request.architectureCommit, architectureDigest: request.architectureDigest, implementationCommit: request.implementationCommit, affectedIdentifiers: [...request.affectedIdentifiers] };
}

export async function validateConformanceRequestFile(filePath, expectedArchitectureCommit = null, expectedArchitectureDigest = null) {
  return validateConformanceRequest(JSON.parse(await readFile(filePath, 'utf8')), expectedArchitectureCommit, expectedArchitectureDigest);
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    const result = await validateConformanceRequestFile(process.argv[2], process.argv[3] ?? null, process.argv[4] ?? null);
